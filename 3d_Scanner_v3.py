# by chris martin 12/6/2020 revised 2/11/2025
# rewritten for:
# Distributor ID: Debian
# Description:    Debian GNU/Linux 11 (bullseye)
# Release:        11
# Codename:       bullseye
# bbio.pwm relaced with pru program
# manythings broke from earlier release
...
#direction=0 # Scanner operates clockwise viewed from top not needed follows sign of angle step
m1=1 # Legacy mode stepper driver step selection
...
#RUNTIME VARIBLES
zaxisindex=10000 # number of scans to take on the zaxis tested up to 6000 so far with angle sensor
Nscancycles=5 # rplidar scan rotations the rplidar module does nto always give the same angles for measurements, a higher number here
...
while not loopend:
    ...
    measangle=(angleprime-startangleprime+360*rollover)/driveratio  #calculate zaxis angle
    lastangle=rawangle
    if not oneshot:
        startposangle=z_angle  #was posangle no we'll use the accuracy of the pru to set angle
        oneshot=True
    
    
    zaxisangle=z_angle # convert to radians
    ...
    print ("calculated angle: ",'{:14.6f}'.format(math.degrees(calczaxisangle))," Z_angle:",'{:14.6f}'.format(math.degrees(z_angle)), " measured angle:",'{:14.6f}'.format(measangle), end='\r')       
    
    calczaxisangle=calczaxisangle+zaxisanglestep #count up zaxis revolutions
    z_angle=z_angle+zaxisanglestep
    command_bits=mc.zaxis(command_bits,math.degrees(zaxisanglestep),zaxisspeed,zacceltime,m1,m2,multiplier)
    if zaxisangle > ((2*pi)): # z axis range 1x 180deg 2x360
        loopend=1
    
f.close() #close xyz file
g.close()
lidar.set_motor_pwm(0)  
command_bits=mc.zaxis(command_bits,-360,15,2,m1,m2,multiplier)
