...
class anglesensor:
    ...
    def __init__(self, port0, port1, spimode):
        subprocess.run("lsmod | grep ^spidev || sudo modprobe spidev || true", shell=True, check=True)  
        subprocess.run("sudo chgrp gpio /dev/spidev*", shell=True, check=True)  
        subprocess.run("sudo chmod 660 /dev/spidev*", shell=True, check=True)  
